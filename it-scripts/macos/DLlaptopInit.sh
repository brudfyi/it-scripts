...
echo "opening Dapper Labs Jamf enroll"
sleep1
open https://dapperlabs.jamfcloud.com/enroll -a Safari
...
if ! open -Ra "Discord">/dev/null; then brew install discord cask; fi
if ! open -Ra "Slack">/dev/null; then brew install slack cask; fi
if ! open -Ra "Google Drive File Stream">/dev/null; then brew install google-drive-file-stream cask; fi
...
if ! open -Ra "Google Chrome">/dev/null; then brew install google-chrome cask; fi
if ! open -Ra "Perimeter81">/dev/null; then brew install Perimeter81 cask; fi
if ! open -Ra "iTerm">/dev/null; then brew install iterm2 cask; fi
if ! open -Ra "Spotify">/dev/null; then brew install spotify cask; fi
...
if ! open -Ra "1Password 6">/dev/null; then brew install 1password cask; fi
...
dockutil --add /Applications/Perimeter\ 81.app 
